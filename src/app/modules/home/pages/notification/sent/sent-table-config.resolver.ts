...
export class SentTableConfigResolver
  implements
    Resolve<
      EntityTableConfig<NotificationRequest, PageLink, NotificationRequestInfo>
    >
{
  ...
  constructor(
    ...
    this.config.entityTranslations = entityTypeTranslations.get(
    ...
    this.config.deleteEntityTitle = () =>
      this.translate.instant("notification.delete-request-title");
    this.config.deleteEntityContent = () =>
      this.translate.instant("notification.delete-request-text");
    this.config.deleteEntitiesTitle = (count) =>
      this.translate.instant("notification.delete-requests-title", { count });
    this.config.deleteEntitiesContent = () =>
      this.translate.instant("notification.delete-requests-text");

    this.config.deleteEntity = (id) =>
      this.notificationService.deleteNotificationRequest(id.id);
    this.config.entitiesFetchFunction = (pageLink) =>
      this.notificationService.getNotificationRequests(pageLink);
    ...
    this.config.handleRowClick = (event, entity) => {
      if (
        (event.target as HTMLElement).getElementsByClassName("stats").length ||
        (event.target as HTMLElement).className === "stats"
      ) {
    ...
    this.config.columns.push(
      new DateEntityTableColumn<NotificationRequestInfo>(
        "createdTime",
        "common.created-time",
        this.datePipe,
        "170px"
      ),
      new EntityTableColumn<NotificationRequestInfo>(
        "status",
        "notification.status",
        "15%",
        (request) =>
          `<span style="display: flex;">${this.requestStatus(
            request.status
          )}${this.requestStats(request.stats)}</span>`,
        (request) => this.requestStatusStyle(request.status)
      ),
      new EntityTableColumn<NotificationRequest>(
        "deliveryMethods",
        "notification.delivery-method.delivery-method",
        "15%",
        (request) =>
          request.deliveryMethods
            .map((deliveryMethod) =>
              this.translate.instant(
                NotificationDeliveryMethodTranslateMap.get(deliveryMethod)
              )
            )
            .join(", "),
        () => ({}),
  ...
  private configureCellActions(): Array<
    CellActionDescriptor<NotificationRequestInfo>
  > {
    return [
      {
        ...
        isEnabled: (request) =>
          request.status !== NotificationRequestStatus.SCHEDULED,
  ...
  private requestStats(stats: NotificationRequestStats): string {
    ...
    let countError = 0;
    Object.keys(stats.errors).forEach(
      (method) => (countError += Object.keys(stats.errors[method]).length)
    );
    if (countError === 0) {
      return "";
    }
    return `<div style="border-radius: 12px; height: 24px; line-height: 24px; padding: 0 10px; width: max-content; cursor: pointer;
                ...
                ${this.translate.instant("notification.fails", {
                  count: countError,
                })} >
            </div>`;
  }
  ...
  private openStatsErrorDialog(
    ...
    this.dialog
      .open<SentErrorDialogComponent, NotificationRequestErrorDialogData, void>(
        SentErrorDialogComponent,
        {
          disableClose: true,
          panelClass: ["tb-dialog", "tb-fullscreen-dialog"],
