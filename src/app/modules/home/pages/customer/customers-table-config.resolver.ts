...
export class CustomersTableConfigResolver
  implements Resolve<EntityTableConfig<Customer>>
{
  private readonly config: EntityTableConfig<Customer> =
    new EntityTableConfig<Customer>();
  ...
  constructor(
    ...
    this.config.columns.push(
      new DateEntityTableColumn<Customer>(
        "createdTime",
        "common.created-time",
        this.datePipe,
        "150px"
    ...
    this.config.cellActionDescriptors.push(
      ...
      },
      {
        ...
        onAction: ($event, entity) =>
          this.manageCustomerDevices($event, entity),
      },
      {
        ...
        isEnabled: (customer) => true,
        onAction: ($event, entity) =>
          this.manageCustomerDashboards($event, entity),
      }
    );
    ...
    this.config.deleteEntityTitle = (customer) =>
      this.translate.instant("customer.delete-customer-title", {
        customerTitle: customer.title,
      });
    this.config.deleteEntityContent = () =>
      this.translate.instant("customer.delete-customer-text");
    this.config.deleteEntitiesTitle = (count) =>
      this.translate.instant("customer.delete-customers-title", { count });
    this.config.deleteEntitiesContent = () =>
      this.translate.instant("customer.delete-customers-text");

    this.config.entitiesFetchFunction = (pageLink) =>
      this.customerService.getCustomers(pageLink);
    ...
    this.config.deleteEntity = (id) =>
      this.customerService.deleteCustomer(id.id);
    this.config.onEntityAction = (action) =>
      this.onCustomerAction(action, this.config);
    this.config.deleteEnabled = (customer) =>
      customer &&
      (!customer.additionalInfo || !customer.additionalInfo.isPublic);
    this.config.entitySelectionEnabled = (customer) =>
      customer &&
      (!customer.additionalInfo || !customer.additionalInfo.isPublic);
    this.config.detailsReadonly = (customer) =>
      customer && customer.additionalInfo && customer.additionalInfo.isPublic;
  }
  ...
  private openCustomer(
    $event: Event,
    customer: Customer,
    config: EntityTableConfig<Customer>
  ) {
    if ($event) {
      $event.stopPropagation();
    }
    const url = this.router.createUrlTree([customer.id.id], {
      relativeTo: config.getActivatedRoute(),
    });
