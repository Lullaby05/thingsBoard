...
export class UsersTableConfigResolver
  implements Resolve<EntityTableConfig<User>>
{
  private readonly config: EntityTableConfig<User> =
    new EntityTableConfig<User>();
  ...
  constructor(
    ...
    this.config.columns.push(
      new DateEntityTableColumn<User>(
        "createdTime",
        "common.created-time",
        this.datePipe,
        "150px"
    ...
    this.config.deleteEnabled = (user) =>
      user && user.id && user.id.id !== this.authUser.id.id;
    this.config.deleteEntityTitle = (user) =>
      this.translate.instant("user.delete-user-title", {
        userEmail: user.email,
      });
    this.config.deleteEntityContent = () =>
      this.translate.instant("user.delete-user-text");
    this.config.deleteEntitiesTitle = (count) =>
      this.translate.instant("user.delete-users-title", { count });
    this.config.deleteEntitiesContent = () =>
      this.translate.instant("user.delete-users-text");
    ...
    this.config.onEntityAction = (action) =>
      this.onUserAction(action, this.config);
  ...
  resolve(route: ActivatedRouteSnapshot): Observable<EntityTableConfig<User>> {
    ...
    return this.store.pipe(select(selectAuth), take(1)).pipe(
      tap((auth) => {
        ...
        this.authority = routeParams.tenantId
      ...
      map((parentEntity) => {
        if (this.authority === Authority.TENANT_ADMIN) {
          this.config.tableTitle =
            parentEntity.title +
            ": " +
            this.translate.instant("user.tenant-admins");
        } else {
          this.config.tableTitle =
            parentEntity.title +
            ": " +
  ...
  addUser(): Observable<User> {
    return this.dialog
      .open<AddUserDialogComponent, AddUserDialogData, User>(
        AddUserDialogComponent,
        {
          disableClose: true,
  ...
  private openUser($event: Event, user: User, config: EntityTableConfig<User>) {
    ...
    const url = this.router.createUrlTree([user.id.id], {
  ...
  displayActivationLink($event: Event, user: User) {
    ...
    this.userService
      .getActivationLink(user.id.id)
      .subscribe((activationLink) => {
        this.dialog.open<
          ActivationLinkDialogComponent,
          ActivationLinkDialogData,
          void
  ...
  setUserCredentialsEnabled(
    ...
    this.userService
      .setUserCredentialsEnabled(user.id.id, userCredentialsEnabled)
      .subscribe(() => {
          ...
          new ActionNotificationShow({
            message: this.translate.instant(
              userCredentialsEnabled
                ? "user.enable-account-message"
                : "user.disable-account-message"
            ),
            type: "success",
          })
        );
  ...
  onUserAction(
    action: EntityAction<User>,
    config: EntityTableConfig<User>
  ): boolean {
