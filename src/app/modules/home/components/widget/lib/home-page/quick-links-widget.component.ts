  ...
  implements OnInit, OnDestroy
{
  ...
  menuLinks$(): Observable<Array<MenuSection>> {
    return this.quickLinks
      ? this.menuService.menuLinksByIds(this.quickLinks.links)
      : of([]);
  }
  ...
  edit() {
    this.dialog
      .open<EditLinksDialogComponent, EditLinksDialogData, boolean>(
        EditLinksDialogComponent,
        {
          disableClose: true,
          autoFocus: false,
  ...
  addLink() {
    this.dialog
      .open<AddQuickLinkDialogComponent, any, string>(
      ...
      .subscribe((link) => {
        if (link) {
          ...
          this.cd.markForCheck();
          this.userSettingsService
            .updateQuickLinks(this.quickLinks)
            .subscribe();
        }
      });
  }
