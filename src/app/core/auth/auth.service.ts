...
export class AuthService {
  ...
  private static isTokenValid(prefix) {
    ...
    return (
      clientExpiration && Number(clientExpiration) > new Date().valueOf() + 2000
    );
  }
  ...
  public static getRefreshToken() {
    return AuthService._storeGet("refresh_token");
  }
  ...
  public checkTwoFaVerificationCode(
  ...
  public getUserPasswordPolicy() {
    return this.http.get<UserPasswordPolicy>(
  ...
  public resendEmailActivation(email: string) {
    ...
    return this.http.post(
      `/api/noauth/resendEmailActivation?email=${encodeEmail}`,
      null,
      defaultHttpOptions()
    );
  }
  ...
  public loginAsUser(userId: string) {
    return this.http
      .get<LoginResponse>(`/api/user/${userId}/token`, defaultHttpOptions())
      .pipe(
        tap((loginResponse: LoginResponse) => {
          this.setUserFromJwtToken(
            loginResponse.token,
  ...
  public loadOAuth2Clients(): Observable<Array<OAuth2ClientInfo>> {
    ...
    return this.http
      .post<Array<OAuth2ClientInfo>>(url, null, defaultHttpOptions())
      .pipe(
        catchError((err) => of([])),
        tap((OAuth2Clients) => {
  ...
  public getAvailableTwoFaLoginProviders(): Observable<
    Array<TwoFaProviderInfo>
  > {
    return this.http
      .get<Array<TwoFaProviderInfo>>(
  ...
  public forceDefaultPlace(
    ...
    if (authState && authState.authUser) {
      if (
        ...
        if (
          (this.userHasDefaultDashboard(authState) &&
            authState.forceFullscreen) ||
          authState.authUser.isPublic
        ) {
  ...
  public defaultUrl(
    ...
    if (isAuthenticated) {
      if (authState.authUser.authority === Authority.PRE_VERIFICATION_TOKEN) {
        ...
        !path ||
        path === "login" ||
        this.forceDefaultPlace(authState, path, params)
      ) {
        if (this.redirectUrl) {
        ...
        if (
          ...
          if (this.userHasDefaultDashboard(authState)) {
            ...
            result = this.router.parseUrl(
              `dashboard/${authState.lastPublicDashboardId}`
            );
          }
        }
      }
  ...
  private loadUser(doTokenRefresh): Observable<AuthPayload> {
    ...
    if (!authUser) {
      ...
      const accessToken =
        this.utils.getQueryParam("accessToken") || AuthService.getJwtToken();
      const refreshToken = this.utils.getQueryParam("refreshToken") || AuthService.getRefreshToken();
    ...
    this.validateJwtToken(doTokenRefresh).subscribe(
      () => {
        ...
        if (
          ...
          authPayload.authUser.scopes &&
          authPayload.authUser.scopes.length
        ) {
          authPayload.authUser.authority =
            Authority[authPayload.authUser.scopes[0]];
              ...
              this.loadSystemParams().subscribe(
                (sysParams) => {
                  ...
                  if (
                    ...
                    authPayload.userDetails.additionalInfo.lang
                  ) {
  ...
  private loadSystemParams(): Observable<SysParamsState> {
    return this.http
      .get<SysParams>("/api/system/params", defaultHttpOptions())
      .pipe(
        map((sysParams) => {
  ...
  public refreshJwtToken(
    loadUserElseStoreJwtToken = true
  ): Observable<LoginResponse> {
    ...
    if (this.refreshTokenSubject === null) {
      ...
      if (!refreshTokenValid) {
        ...
        const refreshObservable = this.http.post<LoginResponse>(
          "/api/auth/token",
          refreshTokenRequest,
          defaultHttpOptions()
        );
        refreshObservable.subscribe(
          ...
          () => {
            ...
            this.refreshTokenSubject.error(
              new Error(this.translate.instant("access.refresh-token-failed"))
            );
            this.refreshTokenSubject = null;
          }
        );
  ...
  private userForceFullscreen(authPayload: AuthPayload): boolean {
    return (
      (authPayload.authUser && authPayload.authUser.isPublic) ||
  ...
  private userHasDefaultDashboard(authState: AuthState): boolean {
    if (
      ...
      authState.userDetails.additionalInfo &&
      authState.userDetails.additionalInfo.defaultDashboardId
    ) {
      return true;
    } else {
